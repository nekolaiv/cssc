$(document).ready(function () {
    ...
    function loadApplications(filters = {}) {
        $.post(
            "/cssc/server/staff/application_server.php",
            { action: "read", ...filters },
            function (response) {
                try {
    ...
    // Dynamically Populate Curriculum Filter
    function loadCurriculums() {
        $.post("/cssc/server/staff/application_server.php", { action: "fetch_curriculums" }, function (response) {
            try {
                ...
                curriculums.forEach((curr) => {
                    dropdown.append(`<option value="${curr.id}">${curr.remarks}</option>`);
                });
            } catch (error) {
